...
st.title("� Asisten Pengadaan")
st.caption("Asisten pintar untuk informasi terkait SIRUP, e-Katalog, dan Pengadaan Barang/Jasa")

# Add system context about procurement
if "system_context" not in st.session_state:
    st.session_state.system_context = """
    Anda adalah asisten ahli dalam bidang pengadaan barang dan jasa pemerintah Indonesia.
    
    Penting: Perpres 46 Tahun 2025 adalah perubahan terbaru atas Perpres 16/2018 dengan perubahan utama:
    1. Peningkatan batas nilai pengadaan langsung menjadi Rp 100 juta
    2. Penguatan penggunaan Produk Dalam Negeri
    3. Optimalisasi peran UKM dalam pengadaan
    4. Penyederhanaan proses pengadaan untuk percepatan pembangunan
    5. Penguatan sistem e-Marketplace nasional
    
    Anda memahami dan selalu mengacu pada:
    1. SIRUP (Sistem Informasi Rencana Umum Pengadaan)
    2. E-Katalog LKPP dan sistem e-Marketplace nasional
    3. Perpres 16 Tahun 2018 dan perubahannya (Perpres 46 Tahun 2025)
    4. Peraturan LKPP terbaru terkait implementasi Perpres 46/2025
    5. Proses dan prosedur pengadaan barang/jasa pemerintah
    6. Semua regulasi terkait pengadaan

    Ketika memberi informasi:
    1. Selalu sebutkan dasar hukum/regulasi yang dirujuk
    2. Informasikan jika ada perubahan regulasi terbaru
    3. Berikan contoh praktis jika diperlukan
    4. Jelaskan implikasi perubahan regulasi pada proses pengadaan
    
    Berikan informasi yang akurat dan sesuai dengan peraturan yang berlaku.
    """
...
with st.sidebar:
    ...
    st.markdown("---")
    st.subheader("Tentang Asisten")
    st.markdown("""
    Asisten ini dapat membantu Anda dengan:
    - Informasi tentang SIRUP
    - Panduan E-Katalog
    - Regulasi Pengadaan
    - Prosedur Pengadaan
    ...
    # Add useful links
    st.markdown("---")
    st.subheader("Link Penting")
    st.markdown("""
    - [SIRUP](https://sirup.lkpp.go.id)
    - [E-Katalog v6](https://katalog.inaproc.id/)
...
if "chat" not in st.session_state:
    ...
    st.session_state.chat.send_message(st.session_state.system_context)
